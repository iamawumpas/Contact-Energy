...
**Version Roadmap:** v1.4.x → v1.5.x → v1.6.x → v1.7.x → v1.8.x → v2.0.0  
**Current Phase:** Phase 3 (v1.6.x) - ✅ COMPLETE  
...
### Phase 1 (v1.4.x) - Core Download & Caching ✅
...
**Status:** ✅ COMPLETE - v1.4.0 released
...
**Status:** ✅ COMPLETE - v1.5.9 released with fixed race condition

### Phase 3 (v1.6.x) - ApexCharts Integration ✅
**Scope:** Format data for ApexCharts consumption  
**Exit Criteria:** Users can graph hourly/daily/monthly with paid/free breakdown  
**Status:** ✅ COMPLETE - v1.6.0 released with hourly_data and hourly_free_data attributes
...
## 🔧 Phase 1 (v1.4.x) - Implementation Details
...
- [ ] Handle 401 (expired token) → trigger re-auth
- [ ] Handle 404 (invalid contract_id) → log error
- [ ] Handle 400 (invalid date range) → log and skip
...
- [ ] Implement `load(contract_id)` → read JSON from disk, handle missing file
- [ ] Implement `save(contract_id)` → write JSON to disk, create directory if needed
- [ ] Implement `update_hourly(data)` → merge new hourly records
- [ ] Implement `update_daily(data)` → merge new daily records
- [ ] Implement `update_monthly(data)` → merge new monthly records
- [ ] Implement `prune_hourly(window_days=9)` → remove records older than window
- [ ] Implement `prune_daily(window_days=35)` → remove records older than window
- [ ] Implement `prune_monthly(window_months=18)` → remove records older than window
- [ ] Implement `get_hourly_range()` → return (from_date, to_date)
- [ ] Implement `get_daily_range()` → return (from_date, to_date)
- [ ] Implement `get_monthly_range()` → return (from_date, to_date)
...
- ✅ API calls < 10 on first run, < 3 on subsequent runs
- ✅ Cache persists across restarts
- ✅ Logs are informative and debug-friendly
...
### Phase 3 (v1.6.x) - ApexCharts Integration ✅ COMPLETE

**v1.6.0 Implementation Summary:**

✅ Added `hourly_data` attribute (dict keyed by ISO datetime for paid usage)  
✅ Added `hourly_free_data` attribute (dict keyed by ISO datetime for free usage)  
✅ Created `assets/chart_hourly_usage.yaml` with working ApexCharts card  
✅ Updated `wiki/Dashboards.md` with setup guides and examples  
✅ Documented template variable configuration for multi-account setup  

...
### Fixed
- Fixed a false permanent setup failure when the initial authentication request timed out during integration startup.

### Changed
- Setup now raises `ConfigEntryNotReady` for temporary connection/timeout errors during startup authentication, so Home Assistant automatically retries instead of marking the entry as failed. Genuine authentication failures (invalid credentials) still fail setup as before.
...
### Fixed
- The Home Assistant Energy dashboard now imports paid and free usage at the original hourly timestamps instead of recording one cumulative point at midnight each day.
- Recent hourly statistics are merged with older daily history without overlapping or double-counting energy totals.
...
### Added
- Added monthly cost attributes to the usage sensor and monthly usage chart example
...
- Removed datetimeUTC: true and timezone config that caused inconsistent date shifting across environments
...
### Fixed
- **Critical**: Resolved persistent "Invalid mean type" error by using dictionary unpacking when constructing StatisticMetaData
- Metadata now built as explicit dictionary with only required fields, ensuring `mean_type` is completely absent from the object
- Added graceful fallback for `unit_class` parameter to maintain backward compatibility with Home Assistant 2025.1+
- Statistics import now works across Home Assistant versions 2025.1 through 2026.x without validation errors

## [ 1.7.46 ]

### Fixed
- **Critical**: Fixed "Invalid mean type" error by removing `mean_type=""` from StatisticMetaData. When `has_mean=False`, the `mean_type` parameter must be omitted entirely per Home Assistant requirements
- Statistics import now succeeds without integrity constraint failures or deprecation warnings
- Usage data now imports correctly into Home Assistant statistics database for Energy Dashboard

## [ 1.7.45 ]

### Fixed
- **Critical**: Fixed Home Assistant 2026.11 deprecation warning by specifying `mean_type=""` for statistics without means (`has_mean=False`)
...
### Fixed
- Fixed database constraint error by removing `mean_type=None` from StatisticMetaData
- Home Assistant database requires mean_type to be a valid value or omitted entirely when has_mean=False
- Statistics import now works without deprecation warnings or database errors

## [ 1.7.35 ]

### Fixed
- Added `mean_type=None` to StatisticMetaData to comply with Home Assistant's requirements for external statistics API
- Resolves deprecation warning about missing mean_type parameter (will stop working in Home Assistant 2026.11)

## [ 1.7.33 ]

### Fixed
- Fixed "Invalid timestamp" error in statistics import by using top-of-hour timestamps (00:00:00 UTC) instead of end-of-day (23:59:59)
- Home Assistant's external statistics API requires timestamps with minutes and seconds = 0 for proper validation
...
### Fixed
- Fixed "Invalid statistic_id" error when importing historical statistics
  - Changed metadata from plain dict to `StatisticMetaData` object
  - Home Assistant's `async_import_statistics` requires proper `StatisticMetaData` type
  - Imported `StatisticMetaData` from recorder component

## [ 1.7.27 ]

### Fixed
- Fixed sensor start date initialization logic in statistics import
  - Simplified date finding algorithm to directly use min() on dictionary keys
  - Added error handling for date parsing failures
  - Now correctly identifies earliest date from daily records instead of defaulting to today
  - Added detailed logging showing record count and initialization results
...
### Fixed
- Switched energy sensors to state_class=total and restored last_reset to satisfy Home Assistant validation
- Keeps cumulative tracking for Energy Dashboard while avoiding last_reset errors on total_increasing

## [ 1.7.17 ]

### Fixed
- Removed incompatible last_reset attribute from energy sensors (total_increasing state class does not support last_reset)
- Energy sensors continue to work with total_increasing state class for proper cumulative tracking
...
### Fixed
- Added native_value debug logging to diagnose Energy Dashboard display issues
- Will show actual kWh values being reported to Home Assistant

## [ 1.7.14 ]

### Fixed
- Fixed paid energy calculation: records stored with "paid" field not "paid_total"
- Energy Dashboard now correctly displays paid energy totals (was reading wrong field name)
- Paid energy should now show ~1032 kWh instead of 0 kWh
...
### Fixed
- Added debug logging to energy sensor cumulative total calculation for troubleshooting
...
### Fixed
- Removed duplicate _write_cache() call in save method that was causing FileNotFoundError (temp file renamed twice)
- Cache save now executes exactly once per call as intended
